...
from train_utils import train_and_evaluate, get_config_dir
...
def generate(args, tokenized_datasets):
    config_dir = get_config_dir(args)
    output_dir = f'ckpts/{config_dir}/{args.run}'  # for model ckpts
    from transformers import T5ForConditionalGeneration
    model = T5ForConditionalGeneration.from_pretrained(output_dir)
    tokenizer = AutoTokenizer.from_pretrained(output_dir)

    input_expl = TaskPrefixDataCollator(tokenizer, model).__call__(tokenized_datasets["test"])
    outputs = model.generate(input_ids=input_expl["expl"]["input_ids"], attention_mask=input_expl["expl"]["attention_mask"], 
                          no_repeat_ngram_size=2, max_new_tokens=50, num_beams=5, early_stopping= True)
    
    decoded_sequence = tokenizer.batch_decode(outputs, skip_special_tokens=True)
    from tqdm import tqdm
    with open('./generate/output_2.txt', "a") as f:
        for seq in tqdm(decoded_sequence):
            f.write(seq + "\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_max_len', type=int, default=1000)
    ...
    tokenized_datasets = run(args)
    generate(args, tokenized_datasets)
